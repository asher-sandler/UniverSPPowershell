...
"@
	...
	try
	{
		Send-MailMessage -Encoding ([system.Text.Encoding]::UTF8) -To $target -From "huji-app@ekmd.huji.ac.il" -cc $helpdesk -Subject $subj -body $newtext -smtpserver $smtpserverName -BodyAsHtml -erroraction Stop
...
"@
	...
	try
	{
		Send-MailMessage -Encoding ([system.Text.Encoding]::UTF8) -To $target -From $doNotReply -cc $helpdesk -Subject $subj -body $newtext -smtpserver $smtpserverName -BodyAsHtml -erroraction Stop
		g; log "Successfuly sent Hebrew mail" $logFileName ;w
	}
	catch
	{
...
if(!$xyzlist){logts "Unable to open $xyzlistname. Exiting." $logFileName;exit}
	...
	$cmd = New-Object System.Data.SqlClient.SqlCommand
	$cmd.connection = $conn
	$cmd.commandtext = "select * from [dbo].[production_Pending_Actions_2] WHERE [f_performed] IS NULL;"
							...
							else
							{
								...
								if($ccuname -and $ccuname -ne "")
								{
									...
									if(!$DocLibDone)
									{
										...
										start-sleep 2
										$tLibrary =  $null; $tLibrary =  $Web.Lists[$tLibraryName]; $tLibrary.OnQuickLaunch = $OnQuickLaunchShow
										# remove permissions
