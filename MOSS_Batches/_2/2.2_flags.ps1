...
$cmd.commandtext=@"
UPDATE [dbo].[temp_users_2]
SET f_appended = 1, f_appended_date = GETDATE()
WHERE [ZEHUT] NOT IN (SELECT t.ZEHUT FROM [dbo].[temp_users_2] t INNER JOIN [dbo].[production_users_2] p ON t.ZEHUT = p.ZEHUT);
"@
		$cmd.executenonquery()
		start-sleep 1
		$conn.close()
	...
	try
	{
		...
		$cmd.connection = $conn
$cmd.commandtext=@"
UPDATE [dbo].[production_users_2]
SET f_cancelled = 1, f_cancelled_date = GETDATE()
WHERE ([f_cancelled] IS NULL Or [f_cancelled] = 0 ) AND [ZEHUT] NOT IN (SELECT p.ZEHUT FROM [dbo].[production_users_2] p INNER JOIN [dbo].[temp_users_2] t ON p.ZEHUT = t.ZEHUT);
"@
		$cmd.executenonquery()
		start-sleep 1
		$conn.close()
	...
	try
	{
		...
		$cmd.connection = $conn
$cmd.commandtext=@"
INSERT INTO [dbo].[production_Pending_Actions_2] 
SELECT [ZEHUT], '2' as ACTION_TYPE_ID,Null as ACTION_RESULT_ID, Null as f_performed, Null as f_performed_date  FROM [dbo].[production_users_2] 
WHERE ZEHUT NOT IN (SELECT p.ZEHUT FROM [dbo].[production_users_2] p INNER JOIN [dbo].[temp_users_2] t ON p.ZEHUT = t.ZEHUT);
"@
		$cmd.executenonquery()
		start-sleep 1
		$conn.Close()
...
FROM [dbo].[temp_users_2] t INNER JOIN [dbo].[production_users_2] p ON t.ZEHUT = p.ZEHUT
WHERE 
