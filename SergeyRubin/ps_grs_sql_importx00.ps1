...
if ((Test-Path $sourceFile) -eq $false) {y;"No input file";w;exit} # testing if file exists
$script=$MyInvocation.InvocationName
log "Start time:  <$now>"           $logFileName
log "User:        <$whoami>"        $logFileName
log "Running ON:  <$localcomputer>" $logFileName
log "Script file: <$script>"        $logFileName
log "sourceFile:  <$sourceFile>"    $logfileName
log "inputFile:   <$inputFile>"     $logfileName
log "----------------------------"  $logFileName
...
if($emptyinputtable)
{
	try
	{
		...
		$cmd.connection = $conn
		$cmd.commandtext="DELETE from [dbo].[temp_users_1]"
		$cmd.executenonquery()
...
{ 
	...
	#$line=$inputArray[0]
	#READING LINE DATA
	$cntLines++; log "LINE($cntLines): <$line>`n" $logFileName
		...
		# Check For SEMESTER
		if($vSEMESTER -ne '1'){log ". SEMESTER is not 1. ($vSEMESTER) Skipping the line" $logFileName;continue }
